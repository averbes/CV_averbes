...
const TechStack = () => {
    ...
    return (
        <section id="techstack" className="py-24 bg-[#030303] text-white relative overflow-hidden font-inter">
            ...
            <div className="container mx-auto px-4 relative z-10">
                <div className="max-w-4xl mb-20 text-center mx-auto">
                    <motion.span
                        ...
                        viewport={{ once: true }}
                        className="text-blue-500 font-black tracking-[0.3em] text-[10px] md:text-xs uppercase mb-4 block"
                    >
                        STACK TECNOLÓGICO
                    </motion.span>
                    <motion.h2
                        ...
                        className="text-4xl md:text-6xl font-black font-outfit text-white tracking-tighter mb-6"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {categories.map((category, idx) => (
                        <motion.div
                            ...
                            whileHover={{ y: -5 }}
                            className="bg-zinc-900/40 backdrop-blur-xl border border-white/5 p-8 rounded-[2rem] hover:border-blue-500/30 transition-all duration-500 group relative overflow-hidden"
                        >
                            {/* Inner glow on hover */}
                            ...
                            <h3 className="text-xl font-black mb-8 text-white font-outfit tracking-tight flex items-center gap-3">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {category.techs.map((tech, techIdx) => (
                                    <div
                                        key={techIdx}
                                        className="flex flex-col items-center justify-center p-4 rounded-2xl bg-white/[0.03] border border-white/[0.03] hover:bg-blue-600/10 hover:border-blue-500/20 transition-all duration-300 relative group/tech"
                                    >
                                        ...
                                        <span className="text-[10px] md:text-xs font-bold text-zinc-500 uppercase tracking-wider group-hover/tech:text-white transition-colors text-center truncate w-full">
                                            {tech.name}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <div className="absolute bottom-0 left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-blue-500/20 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500" />
                        </motion.div>
                    ))}
                </div>
            </div>
